...
;(function (root, factory) {
    ...
    var yearReport = {
        ...
        realSend: function (request) {
            if(this.checkSubmitTime() && sbcommon.sbtj_year(request)){//正常申报提交
                $.cookie('lastSubmitTime_'+this.sbzlDm+'_'+this.djxh,new Date().getTime());
                mini.confirm('已完成申报！','点击确定，关闭页面！',function (action) {
                    if(action === 'ok'){
                        var btnCancel=$("#sb_close");
                        if(btnCancel){
                            btnCancel.click();
                        }
                    }
        ...
        checkIsReportWithOthers: function (request) {
            ...
            if(this.reportWithNext){//关联申报，且后续表未申报时
                ...
                if(this.nextReportHd && nextReportSbzlDm){//能取到核定，即判定为后续表还没有申报
                    ...
                    window.location.href = this.reportWithNext.url+postfix+'&v='+(new Date().getTime()+'_2.0.0');//跳转到下一申报界面
                    return true;
                }
            }
        ...
        checkAllPreTableEdited: function () {
            ...
            $.each(this.preTableIds, function (i,bbid) {
                ...
                if(curTable.status === '0' && curTable.needed === '1'){
        ...
        checkSaveCheckData : function () {
            ...
            $.each(Records,function(i,record){
                ...
                if(!_checkAllTableExist(json_record)){
                    return true;
                }
    ...
    var _reportEval = function(str){
        ...
        var newstr = str.replace(/--/g,'+').replace(/STRLEN/g,'strlen').replace(/SUM/g,'sum').replace(/or/g,'||').replace(/AND/g,'&&').replace(/and/g,'&&').replace(/<>/g,'!=').replace(/ROUND/g,'round').replace(/Round/g,'round').replace(/trimleft/g,'TRIMLEFT').replace(/not/g,'!').replace(/OR/g,'||').replace(/ABS/g,'abs').replace(/MAX/g,'max');
        try{
            result = eval(newstr);
        }
        catch(e){
...
yearReport.bindTableBtnEvent = function () {
    ...
    $('#content-table').on('click','.btn[data-type]',function () {
        ...
        if(type === 'edit' || type === 'reedit'){//填写/重填
            ...
            url = bbid+'/'+that.baseFileName+'_'+bbid+'.aspx?edit=Y';
            if(type === 'reedit'){
            ...
            url = bbid+'/'+that.baseFileName+'_'+bbid+'.aspx?modify=Y';
        }else if(type === 'view'){//查看
            url = bbid+'/'+that.baseFileName+'_'+bbid+'.aspx?preview=Y';
        }else if(type === 'delete'){//删除
            if(that.checkAllowDelete(bbid)){
                that.deleteSingleTableData(bbid);
            }
