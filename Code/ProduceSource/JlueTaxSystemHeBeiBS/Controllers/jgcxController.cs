        ...
        public JObject sbqkcx()
        {
            ...
            if (!string.IsNullOrEmpty(zsxmDm))
            {
                ...
                foreach (GDTXUserYSBQC qc in listqc)
                {
                    ...
                    JToken jt = set.getUserYSBQCReportData(qc.Id, qc.sbzlDm, sbxh);
            ...
            {
                JToken jt;
                //listqc = listqc.ToList();
                foreach (GDTXUserYSBQC qc in listqc)
                {
                    if (qc.sbzlDm == "10101")
                    { sbxh = "001";   jt = set.getUserYSBQCReportData(qc.Id, qc.sbzlDm, sbxh);}
                    else { sbxh = "";  jt = set.getUserYSBQCReportData(qc.Id, qc.sbzlDm, out sbxh); }
