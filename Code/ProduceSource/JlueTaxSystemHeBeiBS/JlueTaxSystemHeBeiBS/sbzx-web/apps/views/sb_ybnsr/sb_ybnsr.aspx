    ...
    <script id=list-template type=text/html>
        ...
        <tr class="item {{if obj.status === '1'}} edited {{else if obj.status === '2'}} resumed {{/if}}" data-bbid="{{obj.bbid}}" data-sbbcode="{{obj.sbbcode}}" data-needed="{{obj.needed}}">
            ...
            <td class="table-operation">
                <a class="btn btn-blue sb_edit" data-type="edit">填写</a> {{if obj.bbid !== '401' && obj.bbid !== '402' && obj.bbid !== '403'}}
                <a class="btn btn-blue sb_reedit" data-type="reedit">重填</a> {{/if}}
                <a class="btn btn-orange sb_modify" data-type="modify">修改</a> {{if obj.bbid !== '401' && obj.bbid !== '402' && obj.bbid !== '403'}}
                <a class="btn btn-green sb_view" data-type="view">查看/打印</a> {{/if}} {{if obj.bbid === '404'}}
                <a class="btn btn-gray sb_delete" id="dkl_del">删除</a> {{else if obj.needed !== '1' && obj.bbid !== '401' && obj.bbid !== '402' && obj.bbid !== '403' && obj.bbid !== '405'}}
                <a class="btn btn-gray sb_delete" data-type="delete">删除</a> {{/if}}
            </td>
        </tr> {{/if}} {{/each}}
    </script>
