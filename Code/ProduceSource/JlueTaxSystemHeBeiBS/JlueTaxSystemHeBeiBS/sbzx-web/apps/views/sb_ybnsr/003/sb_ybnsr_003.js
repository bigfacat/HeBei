...
servyouReport.setFromCheckDataConfig = function () {
    $("#003_15_5").val(servyouReport.getValueFromCheckData('082', '7_2')).blur();  //第9栏取值 《附列资料（五）》第2列“本期不动产进项税额增加额”
    $("#003_16_5").val(servyouReport.getValueFromCheckData('082', '7_4')).blur();  //第10栏取值《附列资料（五）》第3列“本期可抵扣不动产进项税额”
};
/*setFromCheckDataConfig_407: function(_this){
    var y13_5 = _this.getValueFromCheckData('001','28_6')*1+_this.getValueFromCheckData('001','28_9')*1;
    $("#407_13_5").val(y13_5).blur();
}*/
...
servyouReport.afterInit = function () {
    ...
    } else if (parent.ybnsr.needInnerSbhy && data087 && !$.isEmptyObject(data087.checkData)) {
        ybnsrService.setDataFromDataMap(data087['checkData']['003']);
    } else if (parent.ybnsr.needInnerYbjc && data086 && !$.isEmptyObject(data086.checkData)) {
