...
function search() {
    ...
    var pageData = { "pageId": pageId, "pageSize": pageSize };
    grid.setUrl('../../../api/yhscx/fplyxxcx.ashx');
    grid.load({
        data: mini.encode({
            map: formData,
            data: pageData
...
//         grid.setData("");
//         if (!resultData.success) {
//             var paramList = resultData.paramList;
//             if (paramList.length > 0) {
//                 if (paramList[0] == "txyzm") {
//                     mini.alert(resultData.messageCode, '提示信息');
//                 }
//             } else {
//                 mini.alert("查询失败，请稍后再试", '提示信息');
//             }
