...
namespace JlueTaxSystemHeBeiBS.Code
{
    ...
    public class CommonMethod
    {
        ...
        /// <returns></returns>
        public static List<GDTXShanXiUserYSBQCReportData> GetReportData(string reportCode, string ysbqcId, string useYSBQCId)
        {
            ...
            List<GDTXShanXiReportDataConfig> configData = new List<GDTXShanXiReportDataConfig>();//需要排除的name(包括等值匹配、公式匹配)
            List<GDTXShanXiReportDataConfig> dzConfigData = new List<GDTXShanXiReportDataConfig>();//等值匹配
            List<GDTXShanXiReportDataConfig> gsConfigData = new List<GDTXShanXiReportDataConfig>();//公式匹配
            List<GDTXShanXiUserYSBQCReportData> dzReportData = new List<GDTXShanXiUserYSBQCReportData>();
            List<GDTXShanXiUserYSBQCReportData> gsReportData = new List<GDTXShanXiUserYSBQCReportData>();
            List<GDTXShanXiUserYSBQCReportData> currentReportData = new List<GDTXShanXiUserYSBQCReportData>();//学员当前报表匹配好的数据集合
            #region 获取期初累计的数据集合
            ...
            {
                configData = JsonConvert.DeserializeObject<List<GDTXShanXiReportDataConfig>>(reportDataConfig.Data.ToString());
                ...
                {
                    dzReportData = JsonConvert.DeserializeObject<List<GDTXShanXiUserYSBQCReportData>>(dzloadData.Data.ToString());
                    if (dzReportData.Count > 0)
                    {
                        foreach (GDTXShanXiReportDataConfig item in dzConfigData)
                        {
                            foreach (GDTXShanXiUserYSBQCReportData data in dzReportData)
                            {
                                if (data.ReportCode == item.FromReportCode)
                                {
                                    ...
                                    for (int i = 0; i < datavalue.Length - 1; i++)
                                    {
                                        ...
                                        {
                                            GDTXShanXiUserYSBQCReportData newData = new GDTXShanXiUserYSBQCReportData();
                                            newData.ReportCode = reportCode;
                                            newData.UserId = CurrentUser.GetInstance().GetCurrentUserId;
                ...
                #endregion
                #region 获取公式匹配数据集合
                foreach (GDTXShanXiReportDataConfig item in gsConfigData)
                {
                    string value = "0";
                    ...
                    {
                        gsReportData = JsonConvert.DeserializeObject<List<GDTXShanXiUserYSBQCReportData>>(gsloadData.Data.ToString());
                    }
                    if (gsReportData.Count != 0)
                    {
                        foreach (string key in name)
                        {
                            foreach (GDTXShanXiUserYSBQCReportData oitem in gsReportData)
                            {
