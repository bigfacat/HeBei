...
/*自定义vtype:12位整数，2位小数*/
mini.VTypes["double12.2ErrorText"] = "请输入数字，整数部分最多12位，且小数部分最多2位";
mini.VTypes["double12.2"] = function (v) {
    var re = new RegExp("^(([-]?[0-9]{1,12}[.]{1}[0-9]{1,2})$|([-]?[0-9]{1,12})$)");
    if (!v || re.test(v)) return true;
    return false;
};

/*自定义vtype:8位整数，8位小数*/
mini.VTypes["double8.8ErrorText"] = "请输入数字，整数部分最多8位，且小数部分最多8位";
mini.VTypes["double8.8"] = function (v) {
    var re = new RegExp("^(([-]?[0-9]{1,8}[.]{1}[0-9]{1,8})$|([-]?[0-9]{1,8})$)");
    if (!v || re.test(v)) return true;
    return false;
};

/*自定义vtype:10位整数，8位小数*/
mini.VTypes["double10.8ErrorText"] = "请输入数字，整数部分最多8位，且小数部分最多8位";
mini.VTypes["double10.8"] = function (v) {
    var re = new RegExp("^(([-]?[0-9]{1,10}[.]{1}[0-9]{1,8})$|([-]?[0-9]{1,10})$)");
    if (!v || re.test(v)) return true;
    return false;
};
