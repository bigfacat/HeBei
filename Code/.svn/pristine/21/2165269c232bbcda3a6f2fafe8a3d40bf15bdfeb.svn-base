using System;
using System.Collections.Generic;
using System.Linq;
using System.Web;

namespace JlueTaxSystemHeBeiGS.Code
{
    class IndexUserysbqc
    {
        public string bbzldm { get; set; }
        public string bbzlmc { get; set; }
        public string bz { get; set; }
        public string jkqx { get; set; }
        public string jkzt { get; set; }
        public string sbfs { get; set; }
        public string sbqx { get; set; }
        public string sbrq { get; set; }
        public string sbzt { get; set; }
        public string ssqq { get; set; }
        public string ssqz { get; set; }
    }

    class CxUserysbqc
    {
        public string userYSBQCId { get; set; }
        public string YSBQCId { get; set; }
        public string HappenDate { get; set; }
        public string bbzlmc { get; set; }
        public string bz { get; set; }
        public string dataFromBz { get; set; }
        public string fjs { get; set; }
        public string pzxh { get; set; }
        public string sblx { get; set; }
        public string sbrq { get; set; }
        public string sbuuid { get; set; }
        public string sbzt { get; set; }
        public string ssqq { get; set; }
        public string ssqz { get; set; }
        public string tbqk { get; set; }
    }
}