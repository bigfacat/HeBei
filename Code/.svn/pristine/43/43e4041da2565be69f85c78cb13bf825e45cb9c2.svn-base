...
namespace JlueTaxSystemHeBeiGS.Code
{
    ...
    public class CommonMethod
    {
        ...
        /// <returns></returns>
        public static List<GTXHeBeiUserYSBQCReportData> GetReportData(string reportCode, string ysbqcId, string useYSBQCId)
        {
            ...
            List<GTXHeBeiReportDataConfig> configData = new List<GTXHeBeiReportDataConfig>();//需要排除的name(包括等值匹配、公式匹配)
            List<GTXHeBeiReportDataConfig> dzConfigData = new List<GTXHeBeiReportDataConfig>();//等值匹配
            List<GTXHeBeiReportDataConfig> gsConfigData = new List<GTXHeBeiReportDataConfig>();//公式匹配
            List<GTXHeBeiUserYSBQCReportData> dzReportData = new List<GTXHeBeiUserYSBQCReportData>();
            List<GTXHeBeiUserYSBQCReportData> gsReportData = new List<GTXHeBeiUserYSBQCReportData>();
            List<GTXHeBeiUserYSBQCReportData> currentReportData = new List<GTXHeBeiUserYSBQCReportData>();//学员当前报表匹配好的数据集合
            #region 获取期初累计的数据集合
            ...
            {
                configData = JsonConvert.DeserializeObject<List<GTXHeBeiReportDataConfig>>(reportDataConfig.Data.ToString());
                ...
                {
                    dzReportData = JsonConvert.DeserializeObject<List<GTXHeBeiUserYSBQCReportData>>(dzloadData.Data.ToString());
                    if (dzReportData.Count > 0)
                    {
                        foreach (GTXHeBeiReportDataConfig item in dzConfigData)
                        {
                            foreach (GTXHeBeiUserYSBQCReportData data in dzReportData)
                            {
                                if (data.ReportCode == item.FromReportCode)
                                {
                                    ...
                                    for (int i = 0; i < datavalue.Length - 1; i++)
                                    {
                                        ...
                                        {
                                            GTXHeBeiUserYSBQCReportData newData = new GTXHeBeiUserYSBQCReportData();
                                            newData.ReportCode = reportCode;
                                            newData.UserId = CurrentUser.GetInstance().GetCurrentUserId;
                ...
                #endregion
                #region 获取公式匹配数据集合
                foreach (GTXHeBeiReportDataConfig item in gsConfigData)
                {
                    string value = "0";
                    ...
                    {
                        gsReportData = JsonConvert.DeserializeObject<List<GTXHeBeiUserYSBQCReportData>>(gsloadData.Data.ToString());
                    }
                    if (gsReportData.Count != 0)
                    {
                        foreach (string key in name)
                        {
                            foreach (GTXHeBeiUserYSBQCReportData oitem in gsReportData)
                            {
