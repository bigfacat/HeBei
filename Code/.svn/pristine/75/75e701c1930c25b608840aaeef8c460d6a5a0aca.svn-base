...
function fbzlAjax(data,whichstep){
    ...
    if(whichstep == 'requestFbzllist'){
        ...
        var url = '/wszx-web/api/get/multifbzllist.ashx';
...
function onRenderOpearte(e) {
    ...
    if(record.scCount == '0'){  //上传数量
        html = '<a href="javascript:previewFbzl(\'' + index +
            '\');"  class="check-info view-pop" style="display: none;">预览</a>&nbsp;&nbsp;<a href="javascript:uploadFbzl(\'' +
...
function localUpload( index ){
    ...
    mini.open({
        url: '/wszx-web/apps/views/public1/fbzl/localUpload.aspx',  //页面地址
        title: '本地上传',      //标题
        iconCls: '',    //标题图标
        width: 900,      //宽度
        height: 580,     //高度
