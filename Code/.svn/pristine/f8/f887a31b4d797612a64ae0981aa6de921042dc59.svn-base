...
StepNav = function () {
    ...
    var _init = function (id, data, stepNav) {
        ...
        if (nsrData != null) {
            lcid = nsrData.nsrsbh + "" + nsrData.djxh; // 暂用nsrsbh
        }
        ...
        if (!!stepnext) {
            stepnext.click(function () {
                ...
                if (_lastStep()) {
                    //将下一步 改成 提交
                    $(this).html("<span class=\"mini-button-text submit-btn  mini-button-icon ico-next pngbg\">提   交</span>");
                }
                if (_closeStep()) {
                    //完结步骤操作按钮
                    $("#close").show();
    ...
    var _nextStep = function () {
        ...
        //业务提交时， 校验纳税人是否与业务发起人一致 （解决业务提交时，纳税人信息不一致，即非发起人的问题）
        if (_lastStep()) {
            if (parent.$('body').attr('isClient') !== "true") {//客户端嵌入的网厅页面提交时获取不到SUI，so不校验
                var nsrData = JSON.parse(SUI.store.get("NsrjbxxVO"));
                if (nsrData != null && lcid != (nsrData.nsrsbh + "" + nsrData.djxh)) {
                    mini.alert("登录超时，请刷新页面重新操作");
                    mini.hideMessageBox(messageid);
                    return;
                }
            }
        ...
        function goNextPage() {//currentFrame, currentStep
            ...
            if (_isInitOver(nextiframe)) {// 如果已经加载完成，直接调用doInitData方法
                ...
                // 如果页面没有加载，则绑定
                $("#" + dts[c + 1].id).bind("load",
						[iframe, nextiframe, false], _doOnload);
            }
        }
