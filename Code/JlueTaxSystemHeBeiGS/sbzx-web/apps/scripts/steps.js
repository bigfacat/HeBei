...
(function ($, undefined) {
    ...
    $.extend($.fn.steps.defaults, {
        ...
        onStepChanging: function (event, currentIndex, newIndex) {
            ...
            if (currentIndex < newIndex && !!stepNav.yltjStep && stepNav.yltjStep === currentIndex) {
                ...
                if (!stepNav.confirmSubmit) {
                    scrollTo(0, 0);
                    mini.confirm('经办人系本人，此项业务真实。我（单位）愿承担由此产生的一切法律责任。是否确认提交？', '提示', function (action) {
                        if (action === 'ok') {
        ...
        onStepChanged: function (event, currentIndex, priorIndex) {
            ...
            // 当前步骤 index 大于 预览提交步骤index，解除导航条点击跳转时间
            if (currentIndex > stepNav.yltjStep) {
                if (currentIndex == (stepNav.yltjStep + 1) && wssqUtil.tjsqResponse.shfsDm == '01') {
                    stepNav.wizard.steps('next');
                } else {
...
$(function () {
    // 添加页面head
    if (!!stepNav.head && !(Tools.getUrlParamByName('client') === 'Y')) {
